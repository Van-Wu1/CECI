...
#如果“为真”，则可以合并存储在输入目录中的不同导入文件，这些文件的文件名以数字 1 开始并按递增顺序排列（例如：way_import1.geojson、way_import2.geojson 等）——可用于同时处理不同的区域，或者处理无法在一个文件中下载的较大区域。
...
#multiple input files can be merged to one single input (这个位置应该是读取并合并输入数据)
if multi_input:
    input_data = []
    i = 1
...
if not exists(dir_input + file_format): #单一文件判定
    if multi_input:
    ...
    layer_way_input = QgsVectorLayer(dir_input + file_format + '|geometrytype=LineString', 'way input', 'ogr') #所以就是应该直接到这里
    ...
    #delete unneeded attributes 删除不必要的属性，留下了185列字段，太抽象了
    layer = processing.run('native:retainfields', { 'INPUT' : layer, 'FIELDS' : p.attributes_list, 'OUTPUT': 'memory:'})['OUTPUT']

    #list of new attributes, important for calculating cycling quality index 新字段共54个
    new_attributes_dict = {
    'way_type': 'String',
    'index': 'Int',
    'index_10': 'Int',
    ...
    # 在每一段路线上设置“检查点”（以便在每个检查点都检查是否有近在咫尺或平行的高速公路）
    ...
    # 创建“检查缓冲区”（用于检查在给定距离内是否存在近似/平行的高速公路）
    ...
    QgsProject.instance().addMapLayer(layer_path_points_buffers, False) #是否添加，后面可以当断点用
    ...
    # 对于所有检查点：将附近道路的编号、名称和公路等级保存在一个字典中
    sidepath_dict = {}
    ...
    #-------------------------------------------------------------------------------#
    print(time.strftime('%H:%M:%S', time.localtime()), 'Skip offset geometry, keep original centerlines...')

    # 如果需要，可以在这里加“双向取均值”的逻辑
    # 按 OSM id 或其他唯一标识聚合数值字段
    # 仅当你确认需要双向平均时启用
    aggregate_bidirectional = False  # True 则执行双向聚合

    if aggregate_bidirectional:
        import pandas as pd
        from qgis.core import QgsVectorLayer
        import processing

        # 将 QGIS 图层转成 GeoDataFrame
        features = list(layer.getFeatures())
        cols = [f.name() for f in layer.fields()]
        data = []
        for feat in features:
            row = [feat.attribute(c) for c in cols] + [feat.geometry()]
            data.append(row)

        import geopandas as gpd
        gdf = gpd.GeoDataFrame(data, columns=cols + ['geometry'], crs=layer.crs().authid())

        # 找出需要聚合的数值列
        num_cols = [c for c in gdf.columns if c.startswith("fac_") or c in ["base_index", "index"]]
        agg_dict = {c: "mean" for c in num_cols}
        agg_dict["geometry"] = "first"

        if "id" in gdf.columns:
            gdf = gdf.groupby("id", as_index=False).agg(agg_dict)

        # 写回临时内存图层
        temp_path = "memory:"
        gdf_layer = QgsVectorLayer(gdf.to_json(), "roads_agg", "ogr")
        layer = gdf_layer

    # 否则，不做任何几何偏移或合并，直接用原 layer 进入后续计算
    # 注意：这样 layer 保留了原字段和属性，后续步骤不需要改



    # 2：对沿中心线映射的侧边路径的属性/几何图形进行拆分和移动操作（这个是原有逻辑，由于原模型的目标是可视化，因此考虑到偏移线，在本研究中将跳过）
    ...
    with edit(layer):
        for feature in layer.getFeatures():
            ...
            #-------------
            # 获取单向状态。该状态可以是 oneway_value_list 中的某个值（“单向”适用于所有车辆，也适用于自行车）或者“*_motor_vehicles”（该值仅适用于机动车）
