...
def compute_quality_index(df):
    """
    Compute CQI: treat missing speeds as 0 (slowest), barrier and side as scored.
    """
    w_speed, w_barrier, w_side = 0.4, 0.3, 0.3
    # Fill missing speeds with 0
    df['proc_maxspeed_filled'] = df['proc_maxspeed'].fillna(0)
    # Normalize (0–1)
    df['speed_norm'] = df['proc_maxspeed_filled'].clip(0, 60) / 60
    df['barrier_score'] = df['has_physical_barrier'].astype(int)
    df['side_score'] = df['sidepath_presence'].astype(int)
    df['cqi'] = (
        w_barrier*df['barrier_score'] +
